...
export class Day2Component implements OnInit {
  ...
  antwortberechnen() {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      let low: number = this.input[i * 16];
      let high: number = this.input[i * 16];

      for (let j = 0; j < 16; j++) {
        console.log(this.input[i * 16 + j]);
        if (this.input[i * 16 + j] > high) {
          high = this.input[i * 16 + j];
        }
        else if (this.input[i * 16 + j] < low) {
          low = this.input[i * 16 + j];
        }
      }
      this.ergebnis = this.ergebnis + high - low;
      console.log("rest");
    }
  }
  ...
  antwortberechnen2() {
    ...
    for (let i = 0; i < 16; i++) {
      for (let j = 0; j < 16; j++) {
        let divident: number = this.input[i * 16 + j];
        for (let k = 0; k < 16; k++) {
          if (!(j-k)) continue;
          let divisor: number = this.input[i * 16 + k];
